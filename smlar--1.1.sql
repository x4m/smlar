...
CREATE FUNCTION g_smlar_distance (internal, anyarray, smallint, oid, internal)
	RETURNS float8
	AS 'MODULE_PATHNAME'
	LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE OR REPLACE FUNCTION arraydist(anyarray, anyarray)
	RETURNS	float4
	AS	'MODULE_PATHNAME', 'arraydist'
	LANGUAGE C STRICT IMMUTABLE;

CREATE OPERATOR <-> (
	LEFTARG = anyarray, RIGHTARG =anyarray, PROCEDURE = arraydist,
	COMMUTATOR = '<->'
);
...
AS
	OPERATOR	1	&& (anyarray, anyarray),
	OPERATOR	2	% (anyarray, anyarray),
	OPERATOR	17	<-> (anyarray, anyarray) FOR ORDER BY float_ops,
	FUNCTION	1	gsmlsign_consistent (gsmlsign, internal, int4),
	FUNCTION	2	gsmlsign_union (bytea, internal),
	FUNCTION	3	gsmlsign_compress (internal),
	FUNCTION	4	gsmlsign_decompress (internal),
	...
	FUNCTION	6	gsmlsign_picksplit (internal, internal),
	FUNCTION	7	gsmlsign_same (gsmlsign, gsmlsign, internal),
	FUNCTION	8	g_smlar_distance (internal, anyarray, smallint, oid, internal),
STORAGE         gsmlsign;
